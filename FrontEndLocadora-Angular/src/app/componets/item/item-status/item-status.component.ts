...
export class ItemStatusComponent implements OnInit {
  ...
  changeAtivo(): void{
    this.itemService.changeAtivo(this.item.id).subscribe(resposta => {
      this.toast.success('Item Ativado com sucesso', 'Ativar');
      this.router.navigate(["itens"])
    }, ex => {
      console.log(ex);
      if (ex.error.errors) {
  ...
  findAllTitulos() {
    this.itemService.findAll(true).subscribe(resposta => {
